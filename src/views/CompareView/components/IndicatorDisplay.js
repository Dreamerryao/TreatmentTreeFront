    ...
    <svg width={after ? 200 : 320} height={25 * (d.detailIndex[d.dataset]?.length ?? 1) - 5}>
      {
        !!d.detailIndex[d.dataset] && d.detailIndex[d.dataset].map((v, vid) => {
          return <g>
            ...
            <rect x={(after ? 0 : 120) + width * 0.25} y={25 * vid} width={width * 0.5} height={height} fill="rgba(86,165,92,0.3)"></rect>
            {
              compareNodes.map((data, did) => {
                ...
                return <rect fill={colors[did]} x={after ? 0 : 120} y={25 * vid + 5 + 5 * did} height={5} width={width * (Math.min((value - itemWidthRange[0]) / (itemWidthRange[1] - itemWidthRange[0]), 1))}> </rect>
              })
            }
          </g>
        })
      }
      {
        !!d.detailIndex[d.dataset] && d.detailIndex[d.dataset].map((v, vid) => {
          ...
          return vid === 0 ? null : otherNodes.length && otherNodes.map((data, did) => {
            ...
            const beforeItemWidthRange = [beforeV.bins[0] * 2 - beforeV.bins[1], beforeV.bins[1] * 2 - beforeV.bins[0]];
            const value = !!data.record ? data.record[0][v.column_id] : 0;
            const itemWidthRange = [v.bins[0] * 2 - v.bins[1], v.bins[1] * 2 - v.bins[0]];
            return <line strokeDasharray="5,5" stroke={colors[did]} stroke-width="2" x1={(after ? 0 : 120) + width * (Math.min((beforeValue - beforeItemWidthRange[0]) / (beforeItemWidthRange[1] - beforeItemWidthRange[0]), 1))} y1={25 * (vid - 1) + 5 + 5 * did + 2.5}
